...
install: libudev.so.1
	...
	cp -f libudev.so.1 ${DESTDIR}/usr${LIBDIR}/libudev.so.1.fake
	ln -rsf ${DESTDIR}/usr${LIBDIR}/libudev.so.1.fake ${DESTDIR}/usr${LIBDIR}/libudev.so.1
	ln -rsf ${DESTDIR}/usr${LIBDIR}/libudev.so.1 ${DESTDIR}/usr${LIBDIR}/libudev.so
	mkdir -p ${DESTDIR}/usr${LIBDIR}/pkgconfig
	cp -f libudev.pc ${DESTDIR}/usr${LIBDIR}/pkgconfig
	mkdir -p ${DESTDIR}/usr/include
	cp -f libudev.h ${DESTDIR}/usr/include
uninstall:
	rm -f ${DESTDIR}${LIBDIR}/libudev.so.1.fake ${DESTDIR}${LIBDIR}/libudev.so.1 ${DESTDIR}${LIBDIR}/libudev.so ${DESTDIR}/usr${LIBDIR}/pkgconfig/libudev.pc ${DESTDIR}/usr${LIBDIR}/libudev.so.1.fake ${DESTDIR}/usr${LIBDIR}/libudev.so.1 ${DESTDIR}/usr${LIBDIR}/libudev.so ${DESTDIR}/usr/include/libudev.h
